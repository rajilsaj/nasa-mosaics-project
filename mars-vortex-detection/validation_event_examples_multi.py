#!/usr/bin/env python3
"""
Validation Event Examples (Multiple)
------------------------------------
Generate concise plots for multiple validation windows per class type:
- True Positives (precursor + RF spike above threshold)
- Misses (precursor but probability below threshold)
- False Positives (no precursor but probability above threshold)
...
def plot_examples(ml_df, examples, threshold):
    if not examples:
    ...
    outfile = os.path.join(OUTPUT_DIR, f"validation_event_examples_multi_{threshold:.2f}_{timestamp}.png")
...
def main():
    parser = argparse.ArgumentParser(description='Generate multiple validation window plots per category.')
    ...
    parser.add_argument('--num_examples', type=int, default=3,
                        help='Number of examples to display per category (default: 3)')
    ...
    plot_examples(ml_df, example_dict, args.threshold)
