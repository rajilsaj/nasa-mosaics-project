...
def extract_windows_from_split(ml_df, jackson_df, split_name, window_size, verbose=False):
    ...
    for idx, row in tqdm(jackson_df.iterrows(), total=len(jackson_df), 
        ...
        # Get the position of the FIRST gt_detection_win=True
        first_precursor_position = precursor_region.index[0]
        
        # Go back window_size steps from the FIRST precursor position
        ...
        try:
            ...
            # Label: True if window's right boundary overlaps with vortex region
            # (end_position is just before precursor starts, so label is True)
            window['label'] = True
