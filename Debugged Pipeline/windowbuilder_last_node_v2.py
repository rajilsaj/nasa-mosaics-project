...
STRIDE_NEAR = 4
NEAR_RADIUS = 75
N_FEATURES = 63  # add this so the memmap shape doesn't use a magic number
...
def count_windows_for_file(X: np.ndarray, y: np.ndarray) -> int:
    """Count windows without storing them — needed to pre-allocate the memmap."""
    n = X.shape[0]
    pos_idx = np.where(y > 0)[0]
    near = np.zeros(n, dtype=bool)
    for p in pos_idx:
...
def main() -> None:
    ...
    # ---- Pass 1: count windows so we can pre-allocate disk space ----
    ...
    # ---- Pass 2: generate windows and write straight to disk ----
