...
def find_continuous_regions(binary_array):
    ...
    # Pad array to handle case where array starts or ends with 1
    padded = np.concatenate([[0], binary_array, [0]])
    # Find where it changes from 0 to 1 (start) or 1 to 0 (end)
    changes = np.diff(padded)
    # Start indices are where it changes from 0 to 1
    starts = np.where(changes == 1)[0]
    # End indices are where it changes from 1 to 0
...
def visualize_pressure_changes(data):
    """Visualize different methods of calculating pressure changes."""
...
def main():
    ...
    # Visualize pressure changes
    visualize_pressure_changes(data)
