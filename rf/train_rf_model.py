...
Self-contained version of "core pipeline scripts/train_rf_model.py",
adapted to live in the rf/ folder:

- Dataset paths resolve to the project root automatically, so the script
  runs from anywhere (including from inside this folder via run.py).
- All outputs (model, metadata, results) are written inside rf/models and
  rf/results, keeping Random Forest artifacts separate from the XGBoost
  ones and from the project root.
- Evaluation report format is identical to the original pipeline and to
  the XGBoost implementation, so all results are directly comparable.

Model export formats (adapted for Random Forest):
- joblib .pkl  — the standard scikit-learn persistence format
- metadata .json — hyperparameters, feature list, and metrics
- feature_importance .csv — impurity-based importances
"""
...
class Config:
    ...
    # File paths (same datasets as the original pipeline)
    ...
    # Output directories (kept inside the rf folder)
    OUTPUT_DIR = os.path.join(SCRIPT_DIR, "models")
    RESULTS_DIR = os.path.join(SCRIPT_DIR, "results")

    # Random Forest parameters — identical to the original pipeline
    RF_PARAMS = {
        'n_estimators': 100,
        'max_depth': 15,
...
def train_random_forest(X_train, y_train):
    ...
    Returns:
        Trained RandomForestClassifier, training time in seconds
    """
...
def evaluate_model(model, X, y, split_name="Validation"):
    ...
    Identical report format to the original pipeline and the XGBoost
    implementation so all models can be compared line by line.
...
def analyze_feature_importance(model, feature_names):
    ...
    Random Forest exposes impurity-based importance: how much each
    feature reduced Gini impurity across all splits in all trees,
    ...
    """
    print(f"\n{'='*70}")
    print("FEATURE IMPORTANCE ANALYSIS (impurity-based)")
    print(f"{'='*70}")
...
def save_model(model, feature_names, training_time, metrics):
    """
    Save trained model and metadata in Random-Forest-appropriate formats:

    - joblib .pkl: the standard persistence format for scikit-learn
      models (Random Forest has no native portable format like XGBoost's
      .json — pickle via joblib is the canonical choice).
    - metadata .json: hyperparameters, feature list, timing, and
      validation metrics, for record-keeping and comparison.
    ...
    # Save model (joblib pickle — canonical scikit-learn format)
...
def main():
    ...
    importance_path = os.path.join(Config.RESULTS_DIR, "rf_feature_importance.csv")
    os.makedirs(Config.RESULTS_DIR, exist_ok=True)
